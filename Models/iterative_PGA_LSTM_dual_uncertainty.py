...
# %% Compute percent z_scores <=2 sigma or 95% empirical coverage
def compute_percent_z_score(mean,std,test_y):
    mask=test_y[:,:,1];
    z_score=np.abs((test_y[:,:,0]-mean)/std);
    z_score=z_score[mask==1];
    tot=len(z_score[z_score>2])/len(z_score);
    return (1-tot);
...
def PGA_Rec_LSTM_train_test(iteration, tr_frac, val_frac, patience_val, num_epochs, batch_size, lstm_nodes, feedforward_nodes, mask_value, drop_frac, lstm_bias, 
                            n_nodes, use_GLM, use_temporal_feature, lamda_reg, shuffle, lamda_main, lamda_aux, tol, pad_steps, hold_out_frac=0.1):
    ...
    #creating train / hold-out validation splits
    train_size=train_X.shape[0]
    n_obs=(int)((1-hold_out_frac)*train_size);
    val_hold_X, val_hold_y_temperature, val_hold_y_glm=train_X[n_obs:,:,:], train_y_temperature[n_obs:,:,:], train_y_glm[n_obs:,:]
    train_X, train_y_temperature, train_y_glm = train_X[:n_obs,:,:], train_y_temperature[:n_obs, :, :], train_y_glm[:n_obs, :]
    ...
    results_dir = '/home/karpatne/Documents/Lake Temperature Modelling/Results/SDM_results/PGA_rectifier_LSTM_dual/'
    ...
    val_hold_y_temperature=transform_3d_to_2d(val_hold_y_temperature);
    ...
    val_hold_y_density=np.zeros(np.shape(val_hold_y_temperature))
    
    train_y_density[:,1], test_y_density[:,1] , val_hold_y_density[:,1] = train_y_temperature[:,1], test_y_temperature[:,1], val_hold_y_temperature[:,1]
    train_y_density[:,0], test_y_density[:,0] , val_hold_y_density[:,0] = density(train_y_temperature[:,0]), density(test_y_temperature[:,0]), density(val_hold_y_temperature[:,0]) 
    ...
    val_hold_y_density[:,0]= (val_hold_y_density[:,0]-mean) / std;
    ...
    [val_hold_y_density,val_hold_y_temperature]=masking_output_labels(val_hold_y_density,val_hold_y_temperature,mask_value)
    ...
    val_hold_y_density=transform_2d_to_3d(val_hold_y_density,depth_steps); 
    val_hold_y_temperature=transform_2d_to_3d(val_hold_y_temperature,depth_steps); 
    ...
    [val_hold_X_pad,val_hold_y_density_pad,val_hold_y_temperature_pad]=edge_padding(val_hold_X,val_hold_y_density,val_hold_y_temperature,pad_steps);
    ...
    test_pred_uq_std = test_pred_do.std(axis=-1).transpose()
    
    train_pred_uq_mean = train_pred_do.mean(axis=-1).transpose()
    train_pred_uq_std = train_pred_do.std(axis=-1).transpose()
    
    [val_rmse_dropout, val_pred_do, val_y1] = dropout_prediction(model,1,val_hold_X_pad, val_hold_y_temperature_pad[:,pad_steps:,:], pad_steps);
    inherent_noise=np.square(val_rmse_dropout)
    
    total_test_uncertainty=np.sqrt(np.square(test_pred_uq_std)+inherent_noise)
    total_train_uncertainty=np.sqrt(np.square(train_pred_uq_std)+inherent_noise)
    
    test_percent_z_score_total_uncertainty=compute_percent_z_score(test_pred_uq_mean,total_test_uncertainty,test_y1)
    test_percent_z_score_model_uncertainty=compute_percent_z_score(test_pred_uq_mean,test_pred_uq_std,test_y1)
    
    print('Test percent z score with total uncertainty : '+str(test_percent_z_score_total_uncertainty));
    print('Test percent z score with model uncertainty : '+str(test_percent_z_score_model_uncertainty));
    
    train_percent_z_score_total_uncertainty=compute_percent_z_score(train_pred_uq_mean,total_train_uncertainty,train_y1)
    train_percent_z_score_model_uncertainty=compute_percent_z_score(train_pred_uq_mean,train_pred_uq_std,train_y1)
    
    print('Train percent z score with total uncertainty : '+str(train_percent_z_score_total_uncertainty));
    print('Train percent z score with model uncertainty : '+str(train_percent_z_score_model_uncertainty));
    ...
    spio.savemat(results_name,{'test_pred_temperature':test_pred1,
                               ...
                               'test_pred_uq_mean':test_pred_uq_mean,
                               'test_pred_model_uncertainty':test_pred_uq_std,
                               'total_test_uncertainty':total_test_uncertainty,
                               'train_pred_uq_mean':train_pred_uq_mean,
                               'train_pred_model_uncertainty':train_pred_uq_std,
                               'total_train_uncertainty':total_train_uncertainty,
                               'test_percent_z_score_total_uncertainty':test_percent_z_score_total_uncertainty,
                               'test_percent_z_score_model_uncertainty':test_percent_z_score_model_uncertainty,
                               'train_percent_z_score_total_uncertainty':train_percent_z_score_total_uncertainty,
                               'train_percent_z_score_model_uncertainty':train_percent_z_score_model_uncertainty
                               })
